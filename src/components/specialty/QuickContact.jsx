  ...
  return (
    <aside className="overflow-hidden rounded-[24px] border border-slate-100 bg-white shadow-[0_18px_50px_rgba(15,23,42,0.06)]">
      <div
        ...
        style={{
          background:
            'linear-gradient(135deg, #FFFFFF 0%, var(--brand-bg-soft) 60%, var(--brand-bg-ultra) 100%)',
        }}
      >
        <div className="pointer-events-none absolute -right-10 -top-10 h-28 w-28 rounded-full bg-[rgb(var(--brand-med)/0.1)] blur-2xl" />
        <div className="relative z-10">
          <p className="text-[10px] font-semibold uppercase tracking-[0.3em] text-[rgb(var(--brand-med))]">
            {eyebrow}
          </p>
          <h3 className="mt-2 text-[20px] font-light leading-snug text-slate-900">
            {title} <span className="font-medium text-[rgb(var(--brand-dark))]">{titleAccent}</span>
          </h3>
        </div>
      </div>
      ...
      <div className="px-2 py-2">
        {contactItems.map((item, idx) => (
          <motion.a
            ...
            className="group flex items-center gap-4 rounded-2xl px-4 py-4 transition-all duration-300 hover:bg-[var(--brand-bg-soft)]"
          >
            ...
            <ArrowUpRight className="h-3.5 w-3.5 shrink-0 text-slate-300 transition-all duration-300 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 group-hover:text-[rgb(var(--brand-med))]" />
          </motion.a>
        ))}
      </div>
